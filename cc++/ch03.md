...
::: {.tip}
**Trap:** Do *not* ship `h1 ^ (h2 << 1)` as your real hash combiner.
XOR is associative and self-inverse, which means hashes that share bit patterns collapse against each other --- combining `(0xAAAA, 0x5555)` and `(0x5555, 0xAAAA)` into the same value, for example.
...
- **Hand-rolled mixing** along the lines of `seed ^= h + 0x9e3779b9 + (seed << 6) + (seed >> 2);` (Boost's actual formula) avoids the collapse but should be copied from a known-good source rather than reinvented.
