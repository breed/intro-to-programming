...
The critical difference from `printf` is that `scanf` needs the **address** of each variable, not the value.
You pass addresses using the `&` operator:
...
Without the `&`, `scanf` would receive the current (uninitialized) value of `age` instead of its address, and the result is undefined behavior.

For strings, you pass the array name directly — recall from the differences table that C strings are just `char` arrays, and an array name already decays to a pointer:
...
::: {.tip}
**Trap:** `%s` reads only until the first whitespace character.
